...
.PHONY: migrate
.PHONY: upgrade
upgrade: $(CLI) ## Update CLI and action pins together, then show the diff (VERSION=x.y.z optional)
	PYTHONDONTWRITEBYTECODE=1 $(CLI) upgrade --to "$(or $(VERSION),latest)" --write
